"use client"

import { useState } from "react"

interface PotSelectorProps {
  selectedPot: "classic" | "modern" | "ceramic" | "terracotta"
  onSelectPot: (pot: "classic" | "modern" | "ceramic" | "terracotta") => void
}

export function PotSelector({ selectedPot, onSelectPot }: PotSelectorProps) {
  const [isOpen, setIsOpen] = useState(false)

  const pots = [
    { id: "classic" as const, icon: "🪴", name: "經典" },
    { id: "modern" as const, icon: "⬜", name: "現代" },
    { id: "ceramic" as const, icon: "🔵", name: "陶瓷" },
    { id: "terracotta" as const, icon: "🟠", name: "陶土" },
  ]

  const currentPot = pots.find((p) => p.id === selectedPot)

  const handleSelect = (potId: "classic" | "modern" | "ceramic" | "terracotta") => {
    onSelectPot(potId)
    setIsOpen(false)
  }

  return (
    <div className="relative flex flex-col items-center w-full">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/40 backdrop-blur-sm border border-plant-secondary/30 hover:bg-white/60 transition-all shadow-sm"
      >
        <span className="text-xl">{currentPot?.icon}</span>
        <svg
          className={`w-3 h-3 text-foreground/50 transition-transform ${isOpen ? "rotate-180" : ""}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
        </svg>
      </button>

      {isOpen && (
        <div className="absolute top-full mt-3 left-1/2 -translate-x-1/2 w-[90vw] max-w-md z-50 animate-in slide-in-from-top-2 fade-in duration-200">
          <div className="relative">
            <div className="overflow-x-auto pb-2 scrollbar-hide">
              <div className="flex gap-4 px-4 py-3 bg-white/90 backdrop-blur-md rounded-full shadow-xl border border-plant-secondary/20 w-max mx-auto">
                {pots.map((pot) => (
                  <button
                    key={pot.id}
                    onClick={() => handleSelect(pot.id)}
                    className={`transition-all ${selectedPot === pot.id ? "scale-110" : "hover:scale-105"}`}
                  >
                    <div
                      className={`w-14 h-14 rounded-full flex items-center justify-center text-2xl transition-all ${
                        selectedPot === pot.id
                          ? "bg-plant-accent/30 ring-2 ring-plant-accent shadow-lg"
                          : "bg-white/60 hover:bg-plant-accent/10"
                      }`}
                    >
                      {pot.icon}
                    </div>
                  </button>
                ))}
              </div>
            </div>
          </div>
        </div>
      )}
    </div>
  )
}
